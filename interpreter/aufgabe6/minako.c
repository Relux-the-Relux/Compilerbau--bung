...
/// read local variable: store the variable into vm->eax
static void
execLocVar(const syntree_node_t* node)
{
    if (vm->ebp + node->value.variable >= vm->stack + 1024) {
        fprintf(stderr, "stack overflow\n");
        exit(-1);
    }
	vm->eax = vm->ebp[node->value.variable];    // node->value.variable is the variable index
    TRACE_VALUE(vm->eax);
}
...
execCall(const syntree_node_t* node)
{
    ...
    syntree_node_t *arg_node = nodeFirst(node);     // prepare parameter over the top of the stack, ebp and esp cannot be updated yet.
    syntree_node_t *arg_iterator = nodeFirst(arg_node);
    
    minako_value_t *temp_ebp = vm->ebp;             // store ebp
    minako_value_t *temp_esp = vm->esp;             // store esp
    
    while (!nodeSentinel(arg_iterator)) {
        dispatch(arg_iterator);                     // calculate argument.
        minako_value_t *param = vm->esp;            // get the address in the stack for this parameter.
        *param = vm->eax;                           // store parameter.
        TRACE_VALUE(vm->eax);
        vm->esp++;                                  // update esp.
        arg_iterator = nodeNext(arg_iterator);      // update iterator.
    }
	/* execute the function body */
    
    vm->ebp = temp_esp;                             // move ebp to the esp of this function, which is also the ebp of the coming function.
    // esp is already updated to the esp of the coming function.
	dispatch(nodeLast(node));
    vm->esp = temp_esp;                             // recover esp
    vm->ebp = temp_ebp;                             // recover ebp
}
...
execSequence(const syntree_node_t* node)
{
    ...
    syntree_node_t* iterator = nodeFirst(node);
    while (!nodeSentinel(iterator)) {
        dispatch(iterator);
        if (vm->returnFlag) {
            vm->returnFlag = 0;
            return;
        } else {
            iterator = nodeNext(iterator);
        }
    }
}
...
execIf(const syntree_node_t* node)
{
	...
	const syntree_node_t* then = nodeNext(test);
    
	/* test if we need to select the else block */
	if (dispatch(test).value.boolean)
		dispatch(then);
    else {  /* TODO else-Fall implementieren */
        const syntree_node_t *else_part = nodeNext(then);
        if (!nodeSentinel(else_part)) {
            dispatch(else_part);
        }
    }
...
{
	/* TODO Implementation */
    const syntree_node_t* init = nodeFirst(node);
    const syntree_node_t* cond = nodeNext(init);
    const syntree_node_t* step = nodeNext(cond);
    const syntree_node_t* exec = nodeNext(step);
    
    for (dispatch(init); dispatch(cond).value.boolean; dispatch(step)) {
        dispatch(exec);
...
execAssign(const syntree_node_t* node)
{
    ...
    syntree_node_t *lhs_node = nodeFirst(node);                     // get the left side node from the syntax tree
    minako_value_t *lhs_addr;
    if (lhs_node->tag == SYNTREE_TAG_GlobVar) {                     /* Global variable */
        lhs_addr = vm->stack + lhs_node->value.variable;            // the address in the global stack for this variable
    } else {                                                        /* Local variable */
        lhs_addr = vm->ebp + lhs_node->value.variable;              // the address in the local stack for this variable
    }
    
    lhs_addr->type = lhs_node->type;                                // store type
    switch (lhs_addr->type) {                                       // store value
        case SYNTREE_TYPE_Boolean:
        lhs_addr->value.boolean = rhs.value.boolean;
        break;
        case SYNTREE_TYPE_Integer:
        lhs_addr->value.integer = rhs.value.integer;
        break;
        case SYNTREE_TYPE_Float:
        lhs_addr->value.real = rhs.value.real;
        break;
        case SYNTREE_TYPE_String:
        lhs_addr->value.string = rhs.value.string;
        default:
...
execLogOr(const syntree_node_t* node)
{
	...
	vm->eax.value.boolean = (dispatch(nodeFirst(node)).value.boolean || dispatch(nodeLast(node)).value.boolean);
}
...
execUminus(const syntree_node_t* node)
{
    ...
    switch (operand.type) {
        ...
        case SYNTREE_TYPE_Void:
        case SYNTREE_TYPE_String:
        assert(!"unexpected type in Uminus operation!");
        break;
    }
}
...
execEqt(const syntree_node_t* node)
{
    ...
    minako_value_t rhs = dispatch(nodeLast(node));
	vm->eax.type = SYNTREE_TYPE_Boolean;
    vm->eax.value.boolean = (lhs.value.integer == rhs.value.integer);
//    switch (nodeFirst(node)->type)
//    {
//    case SYNTREE_TYPE_Boolean:
//        /* TODO Implementation */
//        vm->eax.value.boolean = (lhs.value.boolean == rhs.value.boolean);
//        break;
//
//    case SYNTREE_TYPE_Integer:
//        /* TODO Implementation */
...
//    case SYNTREE_TYPE_Float:
//        /* TODO Implementation */
//        vm->eax.value.boolean = (lhs.value.real == rhs.value.real);
//        break;
//
//    default:
//        assert(!"unexpected type in operation");
...
execNeq(const syntree_node_t* node)
{
    ...
    minako_value_t rhs = dispatch(nodeLast(node));
    vm->eax.type = SYNTREE_TYPE_Boolean;
    vm->eax.value.boolean = (lhs.value.integer != rhs.value.integer);   // just choose a kind of type to compare.
}
